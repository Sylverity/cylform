...
- Added richer native desktop menu actions for opening files, recent files, closing the current molecule tab, exporting PNGs, Settings, standard edit/window roles, and debug-build DevTools.
- Added versioned persisted app settings in `settings.json`, including rendering/export defaults, chemistry and measurement defaults, mouse/zoom preferences, keyboard shortcut remapping, autosave/session/drop behavior, recent-file limits, and app-data diagnostics.
...
- Raised the normal single-structure atom cap to 50,000 atoms while keeping trajectory/computational-output workflows on a future streaming or lazy-loading path.
- Kept the README oriented toward chemists and researchers, with implementation detail moved into `docs/ARCHITECTURE.md`.

### Release Engineering
- Hardened the desktop release workflow and GitHub Actions setup for the private cross-platform validation path.
...
### Planned
- Publish `0.5.0` after private Windows 11, Ubuntu/Debian, and macOS smoke testing.
...
First public release for the v1 publication-workflow milestone.
...
- Toasts, dismissible errors, shortcut dialog, collapsible side panels, and general UI polish for release validation.
