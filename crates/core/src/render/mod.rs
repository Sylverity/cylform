...
pub use mesh::{CylinderMesh, SphereImpostor, InstanceData};
pub use pipeline::{BondPipeline, AtomPipeline};
...
impl Renderer {
    ...
    pub fn render(&mut self, structure: &Structure, camera: &Camera) -> anyhow::Result<()> {
        ...
        let bond_instance_buffer = if !bond_instances.is_empty() {
            Some(self.bond_pipeline.create_instance_buffer(&self.context.device, &bond_instances))
        } else {
            None
        };
        
        let atom_instance_buffer = if !atom_instances.is_empty() {
            Some(self.atom_pipeline.create_instance_buffer(&self.context.device, &atom_instances))
        } else {
            None
        };
        
        // Update camera uniforms
        self.bond_pipeline.update_camera(&self.context.queue, camera);
        self.atom_pipeline.update_camera(&self.context.queue, camera);
    ...
    fn prepare_bond_instances(&self, structure: &Structure) -> Vec<InstanceData> {
        ...
        for bond in &structure.bonds {
            ...
            instances.push(InstanceData::cylinder(
                mid,
                dir,
                length,
                radius,
